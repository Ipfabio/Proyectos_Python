...
def message_probability(user_message, recognized_words, single_response=False, required_word=[]):
    ...
    for word in user_message:
        ...
        porcentage = float(message_certainty) / float(len(recognized_words))
...
def check_all_messages(message):
    ...
    response('Estamos ubicados en la calle wallabis 3312', ['ubicados', 'direccion', 'donde', 'ubicacion'], single_response=True)
    response('Siempre a la orden', ['gracias', 'te lo agradezco', 'thanks'], single_response=True)
    
    best_match = max(highest_prob, key=highest_prob.get)
    #print(highest_prob) Esto imprime las probabilidades de cada respuesta
